...
def save_results(output_files: List[str], reports: List[str]) -> None:
    ...
    if os.path.exists(organized_folder):
        ...
        print("Organized folder moved to result directory.")
...
def main() -> None:
    ...
    logger.info("Starting File Organizer Pipeline...")

    # Load or create extension mapping
    try:
        if args.extensions:
            mapping = {item.split(":")[0]: item.split(":")[1] for item in args.extensions}
            with open("extension.json", "w", encoding="utf-8") as f:
                json.dump(mapping, f, ensure_ascii=False, indent=4)
            logger.info("Extension mapping created from arguments.")
        else:
            with open("extension.json", "r", encoding="utf-8") as f:
                mapping = json.load(f)
            logger.info("Extension mapping loaded from extension.json.")
    except Exception as e:
