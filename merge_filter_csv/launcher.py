...
def main():
    ...
    # Regra: exige pelo menos 2 arquivos
    if len(escolhidos) < 2:
        print("⚠️ Este pipeline requer pelo menos 2 arquivos CSV. Encerrando.")
        return

    # Nome do arquivo de saída
    output_file = input("\nDigite o nome do arquivo de saída [output.csv]: ").strip() or "output.csv"

    # Substituição de valores nulos
    null_replacement = input("Digite a string para substituir valores nulos [Not Informed]: ").strip() or "Not Informed"

    # Aplicar filtros?
    filtro = escolher_opcao(["Sim", "Não"], "Aplicar filtros do filter.json?", "Não")
    ...
    # Montar comando
    cmd = f'python scripts/merge_filter_csv.py --files_csv {" ".join(escolhidos)} --output_csv "{output_file}" --for_NaN "{null_replacement}" {filtro_flag} {report_flags} {log_flag}'
