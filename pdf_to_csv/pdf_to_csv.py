...
def open_pdfs(files: List[str]) -> Dict[str, Any]:
    ...
    for file in files:
        try:
            ...
            failures += 1
            logger.error("Internal PDF engine error in %s: %s", file, e)
            failed_list.append(file)
...
def main() -> None:
    ...
    parser.add_argument("--files", nargs="+", required=True, help="PDF files to process with txt")
    ...
    reports = []
    if args.json: reports.append("report.json")
    if args.txt or (not args.json and not args.txt): reports.append("report.txt")
